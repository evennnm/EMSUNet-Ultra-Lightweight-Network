...
## Abstract
Colorectal cancer prevalence necessitates efficient polyp detection and removal. However, during real-time colonoscopy, missed diagnoses of subtle or small polyps remain a critical clinical challenge, partly due to the lack of instant, accurate visual guidance. Current high-precision segmentation methods suffer from computational complexity, while lightweight models sacrifice accuracy, neither meeting the stringent demands for real-time assistance in clinical practice. 
Given the critical need for computational efficiency in medical image segmentation, this paper introduces an innovative S-M-C-A concept (Split: Channel Split and Dimension Elevation; M-C: Multi-Scale DWConv; Attention: Attention and Channel Fusion). This method leverages channel redundancy to transform single-scale depthwise convolution into multi-scale through splitting, thereby learning diverse multi-scale features to enhance performance. It also employs attention mechanisms to weight scales, thereby boosting representational capacity while minimizing the number of required layers. The SMCA approach concurrently improves performance and reduces parameter count.

Based on the SMCA framework, we propose EMSUNet for real-time polyp segmentation. Its superior computational efficiency surpasses that of all recent polyp segmentation models. The base model presented in this paper is structurally optimized for polyp segmentation tasks, attaining a Dice score of 93.19\%, which is remarkably close to the state-of-the-art (SOTA) performance on the Kvasir dataset. EMSUNet effectively resolves the ``lightweight--accuracy'' paradox, delivering a lightweight solution (1.37~GFLOPs), high accuracy (94.1\% Dice on CVC-ClinicDB), and clinically viable real-time performance (300 FPS on RTX 4060). Furthermore, to explore its potential for handling complex scenarios requiring long-range dependencies, we validate the extensibility of EMSUNet by integrating a Mamba-based module (SMCA-VIM) into its architecture. This enhancement demonstrates a favorable accuracy--efficiency trade-off, boosting performance on challenging datasets like CVC-ColonDB while confirming the robustness and adaptability of the proposed framework as a foundational model for future advancements.
## Usage:
