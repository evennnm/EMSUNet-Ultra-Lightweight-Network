...
from timm.models.layers.squeeze_excite import  SqueezeExcite
...
class SMDC(nn.Module):
    def __init__(self, in_channels, kernel_sizes, stride = 1, activation='relu6'):
        ...
        self.dwconv_group4 = None
        if stride == 2:
            self.dwconv_group4 = nn.Sequential(
                nn.Conv2d(self.in_channels, self.in_channels, 3, stride=2, padding=1,
                          groups=self.in_channels, bias=False),
                nn.BatchNorm2d(self.in_channels),
                act_layer(self.activation, inplace=True)
            )
    ...
    def forward(self, x):
        ...
        group1, group2, group3, group4 = torch.split(x, [channels_per_group] * 4, dim=1)

        group1 = self.dwconvs3(group1)
        group2 = self.dwconvs2(group2)
        group3 = self.dwconvs1(group3)

        # 当步长为2时对group4进行深度卷积处理
        if self.stride == 2 and self.dwconv_group4 is not None:
            group4 = self.dwconv_group4(group4)
...
class SMCA(nn.Module):
    def __init__(self, in_channels, out_channels, stride, kernel_sizes=[3, 5, 7], expansion_factor=2, activation='relu6',):
        ...
        self.newmsdc = SMDC(self.hidden_channels, self.kernel_sizes, self.stride, self.activation)
        ...
        # self.cab=CAB(self.hidden_channels)
        # self.sab=SAB()
    ...
    def forward(self, x):
        ...
        group2 = self.newmsdc(group2)
        group2 = self.SE(group2)
        # group2= self.cab(group2)*group2
        # group2 = self.sab(group2)*group2
...
def SMCALayer(in_channels, out_channels, n=3, stride=1, kernel_sizes=[1, 3, 5],expansion_factor=2, activation='relu6'):
    convs = []
    smca = SMCA(in_channels, out_channels, stride, kernel_sizes=kernel_sizes
                , expansion_factor=expansion_factor, activation=activation)
    convs.append(smca)
    if n > 1:
        for i in range(1, n):
            smca = SMCA(out_channels, out_channels, 1, kernel_sizes=kernel_sizes
                        , expansion_factor=expansion_factor, activation=activation)
            convs.append(smca)
...
class EMDB(nn.Module):
    def __init__(self, in_channels, out_channels, activation='relu'):
        ...
        self.up_dwc = nn.Sequential(
            SMDC(self.in_channels, [3,5,7], 2, activation,),
...
class encoder_EMSUnet(nn.Module):
    def __init__(self, channels=[256, 160, 64, 32], kernel_sizes=[3, 5, 7], expansion_factor=2, activation='relu6'):
        super(encoder_EMSUnet, self).__init__()

        self.SMCA4 = SMCALayer(channels[0], channels[0], n=2, stride=1, kernel_sizes=kernel_sizes,
                               expansion_factor=expansion_factor,activation=activation)

        self.emdb3 = EMDB(in_channels=channels[1], out_channels=channels[0])

        self.SMCA3 = SMCALayer(channels[1], channels[1], n=2, stride=1, kernel_sizes=kernel_sizes,
                               expansion_factor=expansion_factor, activation=activation)

        self.emdb2 = EMDB(in_channels=channels[2], out_channels=channels[1])

        self.SMCA2 = SMCALayer(channels[2], channels[2], n=2, stride=1, kernel_sizes=kernel_sizes,
                               expansion_factor=expansion_factor, activation=activation)

        self.emdb1 = EMDB(in_channels=channels[3], out_channels=channels[2])

        self.SMCA1 = SMCALayer(channels[3], channels[3], n=2, stride=1, kernel_sizes=kernel_sizes,
                               expansion_factor=expansion_factor,activation=activation)
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv2d(12, 24, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(24),
            nn.ReLU(inplace=True),
